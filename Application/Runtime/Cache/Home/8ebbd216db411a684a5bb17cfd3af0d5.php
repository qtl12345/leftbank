    ...
    <script>
        ...
        <section class="Psection MT20">
            ...
            <article class="U-article Overflow">
                ...
                <section class="Mymessage Overflow">
                    ...
                    <?php if(is_array($message)): $i = 0; $__LIST__ = $message;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><p class="FontW"><time><?php echo ($vo["leavedata"]); ?></time><span><?php echo ($vo["content"]); ?></span><input type="hidden" name="hidden<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>"><button type="button"onclick="dmessage('hidden<?php echo ($vo["id"]); ?>')"style="background-color: #4CAF50; /* Green */
                                border: none;
                                color: white;
                                padding: 3px 5px;
                                text-align: center;
                                ...
                                display: inline-block;
                                margin-left: 20px;
                                                                                                                                                                           font-size: 10px;">删除</button></p><?php endforeach; endif; else: echo "" ;endif; ?>
                    <div class="TurnPage">
                        <tfoot>
                            <!--分页显示？-->
                            <tr>
        ...
        <script>
            function leave() {
                ...
                $.ajax({
                    url: "<?php echo U('system/userleave','','');?>",
                    data: {"leave": leave},
                    type: 'post',
                    datatype: 'json',
                    success: function (data) {
            ...
            function dmessage(hiddenid) {
                ...
                $.ajax({
                    url: "<?php echo U('system/dmessage','','');?>",
                    data: {"id": id},
                    type: 'post',
                    datatype: 'json',
                    success: function (data) {
